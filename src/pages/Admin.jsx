...
function AuthGate({ onAuth }) {
  ...
  return (
    <div className={styles.body}>
        <NfcListener onScan={handleNfcScan} disabled={loading || pinFocused} />
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 11px; color: #1D2030; background: #fff; }
  /* ── Branded header bar ── */
  .print-header {
    display: flex; align-items: center; justify-content: space-between;
    background: #1D2030; padding: 12px 24px;
    margin-bottom: 0;
  }
  .print-header img { height: 26px; width: auto; filter: brightness(1.1); }
  .print-header-right { font-size: 10px; color: rgba(255,255,255,0.45); text-align: right; line-height: 1.5; }
  /* ── Content area ── */
  .print-body { padding: 20px 24px 24px; }
  h1 { font-size: 17px; font-weight: 700; color: #1D2030; margin-bottom: 3px; }
  .subtitle { color: #6B7280; font-size: 11px; margin-bottom: 18px; padding-bottom: 12px; border-bottom: 2px solid #7C3AED; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
  th { background: #EDE9FE; text-align: left; padding: 6px 8px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; color: #5B21B6; border-bottom: 2px solid #7C3AED; }
  td { padding: 6px 8px; border-bottom: 1px solid #E5E7EB; vertical-align: top; color: #1D2030; }
  ...
  .section-title { font-size: 13px; font-weight: 700; margin: 20px 0 6px; color: #1D2030; border-bottom: 2px solid #7C3AED; padding-bottom: 4px; }
  .overdue { background: #fff7ed; }
  .print-footer { margin-top: 24px; padding-top: 10px; border-top: 1px solid #E5E7EB; font-size: 10px; color: #9CA3AF; }
  @media print {
...
<div class="print-header">
  <img src="${logoUrl}" alt="RHS CheckMate" onerror="this.outerHTML='<span style=\\"color:#7C3AED;font-size:14px;font-weight:700\\">RHS CheckMate</span>'"/>
  <div class="print-header-right">Media Equipment Checkout System<br/>Riverdale High School · Room 27</div>
</div>
<div class="print-body">
${bodyHtml}
<div class="print-footer">Generated by RHS CheckMate · Riverdale High School Media Department</div>
</div>
<div class="no-print" style="padding:12px 24px;background:#f9fafb;border-top:1px solid #e5e7eb;">
  <button onclick="window.print()" style="padding:8px 18px;background:#7C3AED;color:#fff;border:none;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;">🖨  Print</button>
  <button onclick="window.close()" style="padding:8px 14px;background:transparent;color:#6B7280;border:1px solid #D1D5DB;border-radius:6px;font-size:13px;cursor:pointer;margin-left:8px;font-family:inherit;">Close</button>
</div>
</body></html>`)
  w.document.close()
}
